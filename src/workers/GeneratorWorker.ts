...
function markAsSusspicious(x: number, y: number, field: Uint8Array, cols: number) {
...
type Handler = (e: MessageEvent<IFieldConfigs>) => Handler;

let handler: Handler = (e: MessageEvent<IFieldConfigs>): Handler => {
    ...
    const next = (e: MessageEvent<IFieldConfigs>) => {
        field.fill(0);
        const [rows, cols, freeIndex] = generate();
...
onmessage = function(e: MessageEvent<IFieldConfigs>) {
    handler = handler(e)
};

const initGenerator = ({width: cols, height: rows, mines}: IFieldConfigs, field: Uint8Array): () => [number, number, number] => {
    return () => {
