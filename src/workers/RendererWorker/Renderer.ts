const sourceMapNumbers: Record<number, [number, number, number, number]> = {
    1: [0,   16 * 3, 16, 16],
    2: [16,  16 * 3, 16, 16],
    3: [32,  16 * 3, 16, 16],
    4: [48,  16 * 3, 16, 16],
...
class Renderer {
    ...
    isWin: boolean = false;
    fieldArray!: Uint8Array;
    numbersArray!: Uint8Array;
    fieldViewCacheArray!: Uint8Array;
    cols: number = 0;
    rows: number = 0;
    primaryCanvas!: HTMLCanvasElement;
    primaryContext!: CanvasRenderingContext2D;
    secondaryCanvas!: OffscreenCanvas;
    secondaryContext!: OffscreenCanvasRenderingContext2D;
    highlightCanvas!: OffscreenCanvas;
    highlightContext!: OffscreenCanvasRenderingContext2D;
    texture!: ImageBitmap;
    controlBus!: Uint32Array;
    canvasScale: number = 0;
    initialOrigin: IPoint = { x: 0, y: 0};
    origin: IPoint = { x: 0, y: 0};
    highlighOrigin: IPoint = { x: 0, y: 0 };
    highlightMask: {cols: Record<number, number>, rows: Record<number, number>} = { cols: {}, rows: {} };
    ...
    canvasDirty: {
        ...
        lastX: number,
        lastY: number,
    } = { needUpdate: false, lastX: 0, lastY: 0 };
    fps: { value: number, ticks: number, prev: number, diff: number } =  { value: 0, ticks: 0, prev: 0, diff: 0};

    renderCallback!: (time: number) => void;

    parseEvent(e: MessageEvent<RendererProcessMessage>) {
        this[e.data.type] && this[e.data.type](e.data);
    }

    init(data: RendererProcessMessage) {
    ...
    hasMine(fieldX: number, fieldY: number): boolean {
    ...
    calculateSurroundMines(colFrom: number, colTo: number, rowFrom: number, rowTo: number) {
    ...
    checkChanges(colFrom: number, colTo: number, rowFrom: number, rowTo: number) {
        const field = this.fieldArray;
    ...
    panning(data: PanPostMessage) {
        this.origin.x = data.offset.x;
        this.origin.y = data.offset.y;
    }

    highlight(data: HighlightPostMessage) {
        this.highlighOrigin = data.point;
        this.highlightOn = data.on ?? this.highlightOn;
    ...
    render(time: number) {
        ...
        const lastColTo = this.colTo ?? 0;
        const lastColFrom = this.colFrom ?? 0;
        const lastRowTo = this.rowTo ?? 0;
        const lastRowFrom = this.rowFrom ?? 0;

        let col = Math.abs(Math.min(0, ~~startCol));
        let colTo = Math.min(
            col + (Math.floor(((this.secondaryCanvas.width / this.canvasScale) - Math.max(0, this.origin.x)) / CELL_WIDTH) + COMPLEMENTARY_CELLS), 
            this.cols
        );
        let row = Math.abs(Math.min(0, ~~startRow));
        let rowTo = Math.min(
            row + (Math.floor(((this.secondaryCanvas.height / this.canvasScale) - Math.max(0, this.origin.y)) / CELL_HEIGHT) + COMPLEMENTARY_CELLS), 
            this.rows
        );
        ...
        const offsetX = screenXStart - this.canvasDirty.lastX ?? screenXStart;
        const offsetY = screenYStart - this.canvasDirty.lastY ?? screenYStart;
    ...
    hasUncommtedCommitedChanges():boolean {
    ...
    renderField(column: number, columnTo: number, row: number, rowTo: number,
        numberColumn: number = column, numberRow: number = row) {
    ...
    renderHighlights(screenXStart: number, screenYStart: number, column: number, columnTo: number, row: number, rowTo: number) {
    ...
    renderFPS(time: number, x: number, y: number) {
        this.fps.ticks++;
